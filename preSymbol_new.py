...
train_dataroot = os.getcwd() + "/data/annotated_train_symbol/"
test_dataroot = os.getcwd() + "/data/annotated_test_Equal_boxes/"
symbol_test = {}
images_test = {}
images_test2D = {}
...
def processImage(dataroot, image_name):
    im = cv2.imread(dataroot +image_name)
    im[im >= 127] = 255
    im[im < 127] = 0
    # set the morphology kernel size, the number in tuple is the bold pixel size as test image
...
def main():
    ...
    for train_f in os.listdir(train_dataroot):
#         if f.endswith(".png"):
        marklabel4train(train_dataroot, train_f)
#         total = total + 1
    # print total
    # print (set(symbol.values()))
#     for test_f in os.listdir(test_dataroot): marklabel4test(test_dataroot, test_f)
